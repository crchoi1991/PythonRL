...
class Puzzle:
	def __init__(self, gameCount, shuffleCount):
		# Set window caption
		caption = f"Deep5 : Game {gameCount} shuffle {shuffleCount}"
...
def BuildModel():
	...
	model = keras.Sequential([
		keras.layers.Dense(256, input_dim=Cells, activation='relu'),
...
def GetStatus(board):
	return np.array([ i-board[i]+1 for i in range(Cells) ])
...
while not isQuit:
	...
	while moveCount <= maxMoveCount:
		...
		for k in Actions:
			...
			nst = GetStatus(ps)
			v = model.predict(nst.reshape(1, Cells), verbose=0)[0, 0]
			if maxv < v: a, maxv = k, v
		print(f"({a}, {maxv:.2f})", end=' ')
		v = model.predict(st.reshape(1, Cells), verbose=0)[0, 0]
