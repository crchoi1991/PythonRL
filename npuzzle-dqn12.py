...
CPath = "npuzzle-dqn12/cp_{0:07}.ckpt"
Epochs = 3
BatchSize = 32
Alpha = 0.9
Gamma = 0.97
...
def BuildModel():
	...
	model = keras.Sequential([
		keras.layers.Dense(1280, input_dim=InputSize, activation='relu'),
...
def GetStatus(board, lastAction):
	...
	for r, c in Neighbors:
		...
		if nr < 0 or nr >= VCells or nc < 0 or nc >= HCells:
			...
			s.append(tc-nc)
	for k in range(4):
		s.append(1.0 if lastAction==k else 0.0)
	return s
...
while not isQuit:
	...
	while moveCount <= maxMoveCount:
		...
		st = GetStatus(puzzle.board, lastAction)
		v = model.predict(np.array([st]), verbose=0)[0]
		lastAction = np.argmax(v)
		if st in epx:
			moveCount = maxMoveCount
		else:
			epx.append(st)
			epv.append(v)
			epa.append(lastAction)
		moveCount += 1
		puzzle.action(lastAction)
		if isShow:
	...
	if not isQuit:
		...
		model.fit(xx, yy, epochs=Epochs, batch_size=BatchSize, 
			verbose=0 if gameCount%100!=0 else 1)
