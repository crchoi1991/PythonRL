...
# 상태값 저장용
ss = dict()
with open("puzzle9-td.dat", "r") as f:
	while True:
		line = f.readline()
		if not line: break
		dt = line.split()
...
while not isQuit:
	...
	while moveCount <= maxMoveCount:
		...
		for e in pygame.event.get():
			if e.type == QUIT: isQuit = True
			elif e.type == KEYUP and e.key == K_ESCAPE: isQuit = True
			elif e.type == KEYUP and e.key == K_x: isShow = not isShow
		if isQuit or puzzle.check() == 0: break
	...
	if not isQuit:
		...
		if moveCount <= maxMoveCount:
			moveHistory.append(moveCount)
			solvedCount += 1
		solveRate = solvedCount*100/puzzleCount
		print(f"{solvedCount}/{puzzleCount}:{moveCount}mvs {solveRate:.1f}%",
			f"average : {mean(moveHistory):.2f}")
		if solveRate > 75.0 and puzzleCount >= 10 and shuffleCount < 400:
			solvedCount, puzzleCount, totalMove = 0, 0, 0
			shuffleCount += 1
	if isShow:
...
with open("puzzle9-td.dat", "w") as f:
	for k in ss:
		f.write(f"{k} {ss[k]:.2f}\n")
